...
import { ONRAMP_SRC_URL, ONRAMP_KYB_IS_DONE } from "../ONRAMPER_DETAILS";
...
function Hero({
  ...
  const [raisedInCurrentPhase, setRaisedInCurrentPhase] = useState("22");
  useEffect(() => {
  ...
  const { isAuthenticated, isWeb3Enabled, chainId } = useMoralis();
  ...
  const buyIbatWithBNB = useCallback(() => {
    ...
    let options = {
      abi: ABI_BIGNFT,
      contractAddress: BSC_NFT_CONTRACT_ADDRESS_PHASE[currentPhase],
      functionName: fnName,
  ...
  const toggleCryptoOnRamp = useCallback(() => {
    ...
    let userAddress = localStorage.getItem("NFT_userAddress");
    let nativeC = chain?.nativeCurrency.symbol.toUpperCase();
    let queryParams = `&defaultCrypto=${nativeC}&wallets=${nativeC}:${userAddress}`;
    let newURL = ONRAMP_SRC_URL + queryParams;
  ...
  return (
    <>
      <Flex
        ...
        maxWidth={isLargerThan500 ? "750px" : "100%"}
      ...
      >
        ...
        <Text  textTransform={"uppercase"}
          fontSize={".8rem"}
          fontFamily={"Russo One"}>Balance- 0 $BATS</Text>
        <Text
          textTransform={"uppercase"}
          fontSize={".8rem"}
          fontFamily={"Russo One"}
        ...
        {/* @todo - buying shouldn't be possible after timer is up */}
        <Timer isLargerThan500={isLargerThan500} currentPhase={currentPhase} />
        {isAuthenticated && !currentChainIsSupported && (
          <>
            <Flex direction={"column"} gap={"1rem"}>
              <Text
                ...
                color={"#FEB2B2"}
              >
                Unsupported Chain <br />
                {chain?.name}
              </Text>
              <Text>Click on your wallet address (top-right) to change</Text>
              <List textAlign={"left"}>
                <Text as={"h5"} textAlign={"left"} fontSize="1.3rem">
                  Supported Chains
                </Text>
                {CHAINS_SUPPORTED.map((e) => (
                  <ListItem key={e?.shortName}>
                    {e.shortName.toUpperCase()} - {e.fullName} ({e.chainID})
                  </ListItem>
                ))}
              </List>
            </Flex>
        ...
        {canBuyWithIbat && isAuthenticated && currentChainIsSupported && (
          <Flex
            justifyContent={chainId === "0x38" ? "space-around" : "center"}
            direction={isLargerThan500 ? "row" : "column"}
            gap={"2rem"}
            ...
            {ONRAMP_KYB_IS_DONE && (
              <Flex direction={"column"} gap=".5rem">
                <Text
                  ...
                  borderRadius={"md"}
                >
                  Not enough {chain?.nativeCurrency.symbol.toUpperCase()}? Buy
                  with FIAT(USD, GBP, etc)
                </Text>
               
                <Button variant={"solid"} onClick={toggleCryptoOnRamp}>
                  BUY {chain?.nativeCurrency.symbol.toUpperCase()}
                </Button>
      ...
      {/* THE CRYPTO ONRAMP WHICH IS HIDDEN BY DEFAULT */}
      <Flex
        display={"none"}
        position={"fixed"}
        width={"100vw"}
      ...
      {/* THE CONFIRMATION BEFORE BUYING IBAT */}
